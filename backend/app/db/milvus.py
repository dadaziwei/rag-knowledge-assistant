...
class MilvusManager:
    ...
    def _collection_supports_dynamic_fields(self, collection_name: str) -> bool:
        description = self.client.describe_collection(collection_name)
        return bool(description.get("enable_dynamic_field"))

    def ensure_dynamic_collection(self, collection_name: str, dim: int = 128) -> None:
        """Recreate legacy collections that cannot store metadata fields."""
        if not self.client.has_collection(collection_name):
            self.create_collection(collection_name, dim=dim)
            return

        if self._collection_supports_dynamic_fields(collection_name):
            return

        # Existing demo collections created before the metadata upgrade cannot
        # accept tenant/org fields. Recreate them so uploads can proceed.
        logger.warning(
            f"Recreating legacy Milvus collection without dynamic fields: {collection_name}"
        )
        self.client.drop_collection(collection_name)
        self.create_collection(collection_name, dim=dim)
    ...
    def _build_metadata_filter(self, metadata_filter: dict | None = None) -> str:
    ...
    def search(self, collection_name, data, topk, metadata_filter: dict | None = None):
        ...
        def rerank_single_doc(image_id, data, client, collection_name, filter_expr):
            # Rerank a single document by retrieving its embeddings and calculating the similarity with the query.
            doc_filter = f"image_id in ['{image_id}']"
            if filter_expr:
    ...
    def insert(self, data, collection_name):
        ...
        self.ensure_dynamic_collection(collection_name, dim=len(colqwen_vecs[0]))
